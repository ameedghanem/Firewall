...
int is_proxy_conn_equals(connection_t* c1, connection_t* c2){
	int case1 = c1->proxy_src_ip == c2->src_ip;// && c1->proxy_src_port == c2->proxy_src_port;
	int case2 = c1->proxy_dst_ip == c2->dst_ip && c1->proxy_dst_port == c2->dst_port;
	return case1 && case2;
}
...
connection_t* find_conn_by_proxy(conns_table_t* head, connection_t* conn){
    ...
    else if(is_proxy_conn_equals(head->conn, conn))
    	return head->conn;
    else if(is_proxy_conn_equals(head->rconn, conn))
    	return head->rconn;
...
connection_t* find_conn_by_dst(conns_table_t* head, connection_t* conn){
    ...
    else if(head->conn->dst_ip == conn->dst_ip && head->conn->dst_port == conn->dst_port)
    	return head->conn;
    else if(head->rconn->dst_ip == conn->dst_ip && head->rconn->dst_port == conn->dst_port)
    	return head->rconn;
    else if(!(head->next)){//If it reached the end of our list and didnt find the wanted one, then it returns NULL
        return NULL;
    }
    return find_conn_by_dst(head->next, conn);
}
...
connection_t* find_one_conn(conns_table_t* head, connection_t* conn){
    ...
    if(is_conn_equals(head->conn, conn))
    	return head->conn;
    if(is_conn_equals(head->rconn, conn))
    	return head->rconn;
    if(!head->next){//If it reached the end of our list and didnt find the wanted one, then it returns NULL
        return NULL;
    }
    return find_one_conn(head->next, conn);
}
...
void set_proxy_ports(connection_t* conn){
	if(conn->src_port == FTP_CTRL_PORT){
		...
		conn->proxy_dst_port = conn->src_port;
	}else if(conn->src_port == SMTP_PORT){
		conn->proxy_src_port = 0;
		conn->proxy_dst_port = conn->src_port;
	}else if(conn->dst_port == SMTP_PORT){
		conn->proxy_src_port = SMTP_PROXY_PORT;
		conn->proxy_dst_port = conn->src_port;
	}
...
void set_proxy_conn(connection_t* conn){
	//printk("setting proxy conn: src_ip = %u, dst_ip = %u\n", conn->src_ip, conn->dst_ip);
	if(conn->src_ip == ETH1){
	...
	//printk("proxy_src_ip = %u, proxy_dst_ip = %u, proxy_src_port = %d, proxy_dst_port = %u\n", conn->proxy_src_ip, conn->proxy_src_port, conn->proxy_dst_ip, conn->proxy_dst_port);
}
...
int num_of_entries(conns_table_t* head){
	if(head == NULL)
		return 0;
	return 2 + num_of_entries(head->next);
}


int conns_length(conns_table_t* head){
	return num_of_entries(head) * MAX_CONN_LENGTH;
}
...
void remove_closed_and_timed_out_conns(conns_table_t** conn_tab, unsigned long timestamp){
	conns_table_t *temp, *prev, *x;
	int i=0;
	temp = *conn_tab; prev = *conn_tab; 
	while(temp != NULL){
		if((temp->conn->state == TCP_STT_CLOSED && temp->rconn->state == TCP_STT_CLOSED) || ( (timestamp - temp->timestamp) > 15 && temp->conn->state != TCP_STT_ESTABLISHED && temp->rconn->state != TCP_STT_ESTABLISHED)){
			if(i==0){
				x = *conn_tab;
				*conn_tab = (*conn_tab)->next;
				kfree(x);
				return;
			}
			prev->next = temp->next;
