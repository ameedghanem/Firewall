...
VALID_USERNAME_REGEX = re.compile("^a-z0-9$", re.IGNORECASE)

contains_zip_file = lambda payload: 'Content-Type: application/zip' in payload 
contains_csv_file = lambda payload: 'Content-Type: text/csv' in payload
contains_json_file = lambda request: 'Content-Type: application/json' in request 
is_valid_username = lambda username: VALID_USERNAME_REGEX.match(username).group(0) == username
	...
	def handle(self):
		...
		try:
			...
			while True:
				...
				if len(request) == 0: # this case indicates that the client has ended the connection
					sock.close()
					raise Exception					
				...
				while len(response) != 8192:
					...
					if len(response) <= 8192:
						break
				header = get_http_header(response)
				if contains_zip_file(header):
					...
					sock.close()
					return None
				elif contains_json_file(header):
					content = get_content(response)
					content = json.loads(content)
					if not is_valid_username(content["username"]):
						sock.close()
						return None
